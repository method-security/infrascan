...
The `infrascan discover cdn` command checks if IP addresses or domains belong to known CDN providers.

### Usage

```bash
infrascan discover cdn --domain example.com
```
...
Flags:
      ...
      --fingerprints-file string     Path to CDN fingerprints file (default "/opt/method/infrascan/var/conf/discover/cdn/providers.json")
  -h, --help                        help for cdn
...
The `infrascan discover dns` family of commands provides comprehensive DNS intelligence gathering capabilities.

### Usage

```bash
infrascan discover dns [command]
```
...
The `infrascan discover dns certs` command returns information about the certificate chains that are being leveraged by the specified domain.
...
The `infrascan discover dns forward` command performs forward DNS lookups to identify IPs associated with a domain.

#### Usage

```bash
infrascan discover dns forward --domain example.com
```
...
Usage:
  infrascan discover dns forward [flags]
...
The `infrascan discover dns reverse` command performs reverse DNS lookups on IP addresses or CIDR ranges.

#### Usage

```bash
infrascan discover dns reverse --ip-addresses 8.8.8.8,1.1.1.1
infrascan discover dns reverse --cidr 192.168.1.0/24
```
...
Usage:
  infrascan discover dns reverse [flags]
...
```bash
infrascan discover dns subdomain [command]
```
...
```bash
infrascan discover dns subdomain active --domain example.com --wordlist-size small
```
...
Usage:
  infrascan discover dns subdomain active [flags]
...
```bash
infrascan discover dns subdomain correlation --domains example.com,test.com
```
...
Usage:
  infrascan discover dns subdomain correlation [flags]
...
```bash
infrascan discover dns subdomain passive --domain example.com
```
...
Usage:
  infrascan discover dns subdomain passive [flags]
...
The `infrascan discover ip` command provides IP address and CIDR range intelligence.
...
```bash
infrascan discover ip domain-asn --ip-addresses 8.8.8.8,1.1.1.1
infrascan discover ip domain-asn --cidr 192.168.1.0/24
```
...
Usage:
  infrascan discover ip domain-asn [flags]
...
The `infrascan pentest dns takeover` command detects potential subdomain takeover vulnerabilities.

#### Usage

```bash
infrascan pentest dns takeover --targets https://example.com,subdomain.example.com
```
...
Usage:
  infrascan pentest dns takeover [flags]

Flags:
      --fingerprints-file string   Path to the JSON file containing service fingerprints for takeover detection (default "/opt/method/infrascan/var/conf/pentest/dns/takeover/fingerprints.json")
...
The `infrascan enumerate dns zonetransfer` command attempts DNS zone transfers (AXFR) to enumerate all DNS records if the server allows it.

#### Usage

```bash
infrascan enumerate dns zonetransfer --domains example.com
infrascan enumerate dns zonetransfer --nameserver ns1.example.com
```
...
Usage:
  infrascan enumerate dns zonetransfer [domain...] [flags]
