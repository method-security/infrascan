// Package cmd implements the CobraCLI commands for the infrascan CLI. Subcommands for the CLI should all live within
...
import (
	...
	"github.com/Method-Security/infrascan/internal/config"
...
// NewInfrascan creates a new infrascan struct with the given version. It initializes the root command and all subcommands
...
// InitRootCommand initializes the root command for the Infrascan CLI. This function initializes the root command with a
...
func (a *Infrascan) InitRootCommand() {
	...
	a.RootCmd = &cobra.Command{
		Use:   "infrascan",
		Short: "Perform an infrastructure scan",
		Long:  `Perform an infrastructure scan`,
	...
	a.VersionCmd = &cobra.Command{
		Use:   "version",
		Short: "Prints the version number of infrascan",
		PersistentPreRunE: func(cmd *cobra.Command, _ []string) error {
			return nil
		},
